...
while(length(country.code <- readLines(f, n=1)) > 0) {
  ...
  for (i in 1:length(coverages$online.resource)) {
    ...
    json.list <- c(json.list, list(list(iso=country.code, var=series[1,"identifier"], time=paste(format(as.Date(coverages$start[i]), format="%Y-%m"), "15", sep="-"), value=cellStats(lr, stat="mean"))))
